...
public class SupersonicAirplane extends Airplane{
	...
	public SupersonicAirplane(int speed) {
		...
		// 모두 처리해줘야 합니다.
		// 부모 생성자는 자식쪽에서 호출할 때는
		// 이름이 무조건 super();로 고정입니다.
		// 자식 생성자 내에서 부모생성자 호출이 1순위로 이루어져야 합니다.
		super(speed);
		System.out.println("현재 초음속 비행기 속도 : " + speed);
		}
	// 초음속 비행기는 속도가 900이상, 1300이하인 경우 호출됩니다.
	...
	public void fly() {
		...
		//속도가 900초과 1300이하인 경우 최대속도는 1300 가속은 500씩
			//if(900<speed && speed<=1300)
			if(speed <= 1300) {
				speed += 500;//속도 + 500이 최대속도인 1300을 넘기지 않으면 500을 더해줌
			}else {
				speed = 1300;// 속도 + 500이 최대속도인 1300을 넘기면 1300으로 강제 고정
			}
